...
export default function HostModerationPanel({
  ...
  // ===========================
  const approve = async (reqId: string, accept: boolean) => {
    try {
      await approveSeatApi(roomId, reqId, accept);
      refreshRoom();
    } catch (e) {
  ...
  return (
    <div className="space-y-6 p-3 bg-slate-900 rounded-lg border border-slate-700">
      ...
      <div>
        ...
        {loadingKicks && <div className="text-sm text-slate-500">Loading…</div>}
